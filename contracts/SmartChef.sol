...
contract SmartChef is Ownable {
    ...
    // Deposited amount SATOSHI in SmartChef
	...
	uint256 lpSupply = depositedSatoshi;
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                depositedSatoshi = depositedSatoshi.add(_amount).sub(depositFee).sub(transferTax);
            }else{
                uint256 transferTax = _amount.mul(satoshi.transferTaxRate()).div(10000);
                user.amount = user.amount.add(_amount).sub(transferTax);
                depositedSatoshi = depositedSatoshi.add(_amount).sub(transferTax);
            }
        }        

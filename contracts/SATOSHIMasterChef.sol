...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 satoshiReward = multiplier.mul(satoshiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        satoshi.mint(teamAddr, satoshiReward.div(10));
        satoshi.mint(marketingAddr, satoshiReward.div(90));
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(satoshi)) {
                ...
                uint256 nftFee1 = _amount.mul(satoshi.nftFeeRate1()).div(10000);
                uint256 nftFee2 = _amount.mul(satoshi.nftFeeRate2()).div(10000);
                uint256 nftFee3 = _amount.mul(satoshi.nftFeeRate3()).div(10000);
                uint256 nftFee4 = _amount.mul(satoshi.nftFeeRate4()).div(10000);
                uint256 nftFee5 = _amount.mul(satoshi.nftFeeRate5()).div(10000);
                _amount = _amount.sub(transferTax + nftFee1 + nftFee2 + nftFee3 + nftFee4 + nftFee5);
            }

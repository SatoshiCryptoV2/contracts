...
// File: contracts/SatoshiProfile.sol

pragma solidity ^0.6.0;

/** @title SatoshiProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract SatoshiProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public satoshiToken;
    ...
    uint256 public numberSatoshiToReactivate;
    uint256 public numberSatoshiToRegister;
    uint256 public numberSatoshiToUpdate;
    ...
    constructor(
        IBEP20 _satoshiToken,
        uint256 _numberSatoshiToReactivate,
        uint256 _numberSatoshiToRegister,
        uint256 _numberSatoshiToUpdate
    ) public {
        satoshiToken = _satoshiToken;
        numberSatoshiToReactivate = _numberSatoshiToReactivate;
        numberSatoshiToRegister = _numberSatoshiToRegister;
        numberSatoshiToUpdate = _numberSatoshiToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends SATOSHI to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer SATOSHI tokens to this contract
        satoshiToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberSatoshiToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer SATOSHI token to this address
        satoshiToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberSatoshiToUpdate
        );
    ...
    /**
     * @dev Claim SATOSHI to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        satoshiToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of SATOSHI to register
     * Callable only by owner admins.
     */
    function updateNumberSatoshi(
        uint256 newNumberSatoshiToReactivate,
        uint256 _newNumberSatoshiToRegister,
        uint256 _newNumberSatoshiToUpdate
    ) external onlyOwner {
        numberSatoshiToReactivate = newNumberSatoshiToReactivate;
        numberSatoshiToRegister = _newNumberSatoshiToRegister;
        numberSatoshiToUpdate = _newNumberSatoshiToUpdate;
    }
